...
def filter_player_boxes(result):
    """
    Filter raw YOLO player detections by confidence.

    Parameters:
        result: Ultralytics result object with ``boxes`` and optional ``names``.

    Returns:
        list: Dictionaries containing ``box``, ``class_id``, and ``class_name``.

    Error handling:
        ``None`` results return an empty list. Malformed box objects raise normal
        attribute/index errors so the caller can log the failed inference stage.
    """
    if result is None:
        return []
...
def get_player_bottom_centre(filtered_boxes):
    """
    Convert filtered player boxes into player records with field contact points.

    Parameters:
        filtered_boxes (list): Output from ``filter_player_boxes``.

    Returns:
        list: Player dictionaries containing ``bottom_centre`` coordinates.

    Error handling:
        Non-player/referee classes are ignored. Invalid boxes raise indexing
...
def get_jersey_crop(frame, players):
    """
    Extract upper-torso jersey crops for team colour classification.

    Parameters:
        frame (np.ndarray): BGR image frame.
        players (list): Player dictionaries with ``box`` coordinates.

    Returns:
        list: Same player dictionaries with ``jersey_crop`` and crop box fields.

    Error handling:
        Crop coordinates are clamped to the frame. Invalid frames or boxes raise
        normal NumPy/indexing errors for the caller to log.
    """
...
def extract_jersey_colour(players):
    ...
    Parameters:
        players (list): Player dictionaries with ``jersey_crop`` arrays.

    Returns:
        list: Players enriched with ``jersey_colour`` and candidate colours.

    Error handling:
        Missing or empty crops produce ``None`` colours instead of raising.
    """
...
def build_player_data(frame, player_result):
    """
    Run the player detection/classification preparation pipeline.

    Parameters:
        frame (np.ndarray): BGR frame used for jersey crops.
        player_result: Ultralytics player detection result.

    Returns:
        list: Player records with boxes, bottom-centres, crops, and colours.

    Error handling:
        ``None`` results return an empty list. Lower-level malformed boxes or
        frames raise to the caller.

    Example:
        ``players = build_player_data(frame, next(player_results))``
    """
...
def assign_teams_by_colour(players):
    ...
    Parameters:
        players (list): Player dictionaries with optional ``jersey_colour`` and
            referee class labels.

    Returns:
        list: Players enriched with ``team`` labels of ``0``, ``1``, or ``None``
        for referees/officials.

    Error handling:
        If fewer than two usable colours exist, non-referee players default to
        team ``0`` so downstream code receives a stable structure.
    """
...
def get_offside_players(players, left_offside_line, right_offside_line):
    """
    Return player records positioned between the offside lines.

    Parameters:
        players (list): Player dictionaries with ``bottom_centre``.
        left_offside_line (tuple): ``(x1, y1, x2, y2)`` left boundary.
        right_offside_line (tuple): ``(x1, y1, x2, y2)`` right boundary.

    Returns:
        list: Offside player dictionaries.

    Error handling:
        Invalid line/player structures propagate geometry errors to the caller.
    """
    offside_players = []
...
def get_offside_player_boxes(offside_players):
    """
    Extract bounding boxes from offside player records.

    Parameters:
        offside_players (list): Player dictionaries containing ``box``.

    Returns:
        list: Bounding boxes.

    Error handling:
        Missing ``box`` keys raise ``KeyError`` so tests/callers can catch bad
        player schemas early.
    """
...
def get_offside_team_labels(offside_players):
    """
    Extract team labels from offside player records.

    Parameters:
        offside_players (list): Player dictionaries.

    Returns:
        list: Team labels, using ``None`` where absent.
...
def build_player_coord_dict(players):
    """
    Build the coordinate-to-box mapping used by geometric offside helpers.

    Parameters:
        players (list): Player dictionaries with ``bottom_centre`` and ``box``.

    Returns:
        dict: Mapping ``(x, y)`` bottom-centre coordinates to boxes.

    Error handling:
        Missing required keys raise ``KeyError`` to expose invalid player data.
    """
    player_dict = {}
...
def count_teams_and_refs(players):
    """
    Count players by assigned team and referee/official class.

    Parameters:
        players (list): Player dictionaries with ``team`` and ``class_name``.

    Returns:
        dict: Counts for ``team_0``, ``team_1``, ``unknown_team``, and ``refs``.

    Error handling:
        Missing fields are treated as unknown non-referee players.
    """
    counts = {
        "team_0": 0,
        "team_1": 0,
