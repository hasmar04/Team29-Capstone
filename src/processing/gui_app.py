...
from src.utils import ui_functions as UI
...
class RugbyOffsideGUI:
    ...
    def __init__(self, root):
        ...
        self.batch_reports = []
        self.current_annotated_video = None
    ...
    def _build_ui(self):
        ...
        self.report_details_text.pack(fill="both", expand=True, padx=20, pady=(0, 10))
        self.open_video_button = ctk.CTkButton(
            details_card,
            text="Open Annotated Video",
            command=self.open_current_annotated_video,
            fg_color=MAROON,
            hover_color=MAROON_DARK,
            width=170,
            height=34,
            corner_radius=17,
            font=ctk.CTkFont(family="Arial", size=10, weight="bold"),
        ...
        self.open_video_button.pack(anchor="e", padx=20, pady=(0, 15))
    ...
    def open_current_annotated_video(self):
        """Open the selected report's annotated video in the OS default player."""
        if not self.current_annotated_video or not os.path.exists(self.current_annotated_video):
            messagebox.showwarning("Video unavailable", "No annotated video is available for this report.")
            return

        os.startfile(self.current_annotated_video)
    ...
    def run_auto_mode(self, video_path, fps, output_dir):
        self.progress_queue.put(("progress", "Starting automatic mode processing (headless GUI-safe pipeline)..."))
        self.progress_queue.put(("progress_update", 0))
    ...
    def format_confidence(self, value):
        """Format confidence values consistently for the report UI."""
        try:
    ...
    def display_report_details(self, data):
        ...
        annotated_video = output_files.get("annotated_video")
        report_file = output_files.get("report")
        json_file = output_files.get("json")
        self.current_annotated_video = annotated_video if annotated_video and os.path.exists(annotated_video) else None
        self.open_video_button.configure(state="normal" if self.current_annotated_video else "disabled")
        ...
        lines = [
            ...
            f"Processed: {data.get('processing_date', 'Unknown')}",
            f"Frames: {data.get('total_frames', 'Unknown')} at {data.get('fps', 'N/A')} fps",
            ...
            "OUTPUT FILES",
            "=" * 50,
            f"Annotated video: {annotated_video or 'Not generated'}",
            f"Text report: {report_file or 'Not generated'}",
            f"JSON output: {json_file or 'Not generated'}",
            "",
            "EVENT REVIEW",
            "=" * 50,
        ]
        ...
        if not events:
            lines.append("No candidate events were recorded for this clip.")

        for i, evt in enumerate(events, 1):
            event_frame = evt.get("frame", evt.get("frame_number", "Unknown"))
            event_confidence = evt.get("confidence", evt.get("detection_confidence", "Unknown"))
            counts = evt.get("team_counts", {})
            lines += [
                f"\nEvent #{i}",
            ...
            if evt.get("offside_players"):
                lines.append("  Offside Player Positions:")
                for index, player_info in enumerate(evt["offside_players"], 1):
                    lines.append(
                        f"    {index}. ({player_info.get('x')}, {player_info.get('y')}) "
                        f"confidence {self.format_confidence(player_info.get('confidence'))}"
                    )
            else:
                lines.append("  No players classified as offside.")

        lines += [
            "",
            "REVIEW NOTE",
            "=" * 50,
            "Use the annotated video for visual inspection of field guides, intersection point, offside lines, and player boxes.",
        ]
    ...
    def parse_batch_reports(self, output_dir):
        ...
        json_stems = {
            filename.replace("_analysis.json", "")
        ...
        for filename in files_found:
            if filename.endswith("_analysis.json"):
                ...
                try:
                    report_data = self.parse_json_result(report_path)
                    if not report_data:
                        continue
                    ...
                    self.reports_tree.insert("", "end", values=(
                        report_data.get("video_name", filename),
                        first_event.get("type", "No Events"),
                        self.format_confidence(first_event.get("detection_confidence")),
                        summary.get("total_offside_players", 0)
                    ))
                except Exception as e:
                    self.log_message(f"Error parsing JSON report {filename}: {e}", "ERROR")
                continue

            if not filename.endswith("_analysis_report.txt"):
                continue

            text_stem = filename.replace("_analysis_report.txt", "")
            if text_stem in json_stems:
                continue
    ...
    def parse_json_result(self, report_path):
        """
        Parse a structured batch JSON result for GUI display.

        Parameters:
            report_path (str): Path to ``*_analysis.json``.

        Returns:
            dict: Normalised report data for the reports tree and detail panel.

        Error handling:
            Returns ``None`` when the file cannot be opened or parsed. Details
            are written to the GUI log.
        """
        try:
            ...
            data.setdefault("total_events", summary.get("total_events", len(data.get("events", []))))
            data.setdefault("ruck_count", summary.get("ruck_count", 0))
            data.setdefault("lineout_count", summary.get("lineout_count", 0))
            data.setdefault("total_offside", summary.get("total_offside_players", 0))
            data.setdefault("avg_confidence", summary.get("average_confidence", "N/A"))
            return data
        except Exception as e:
            self.log_message(f"Error parsing JSON report {report_path}: {e}", "ERROR")
            return None
